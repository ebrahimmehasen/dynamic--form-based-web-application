...
// showAppModal / showToast now live in app-modal.js (loaded before this file).
...
function sendData(payload, submitBtn, originalText) {
  ...
  .then(async response => {
    ...
    if (response.ok && result.status === 'success' && result.data && result.data.id) {
      ...
      showSuccessScreen(payload, result.data.id);
    } else {
...
function showSuccessScreen(payload, studentId) {
